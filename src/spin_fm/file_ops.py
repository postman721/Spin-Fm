...
thread and prevents worker lifetimes from retaining interface objects.
"""
...
MAX_TRACKED_DIRECTORY_UPDATES = 256
TRASH_COMMAND_TIMEOUT = 300
TRASH_BATCH_MAX_ITEMS = 96
TRASH_BATCH_MAX_BYTES = 64 * 1024
...
class OperationReport:
    ...
    def record_directory_change(
        ...
        """Retain only the bounded directory changes needed by the UI."""

        if len(self.moved_directories) < MAX_TRACKED_DIRECTORY_UPDATES:
            self.moved_directories.append((source, destination))
...
def _as_sequence(items):
    """Keep an existing sequence instead of copying every bulk-operation item."""
...
def _tracked_path_set(paths: Collection[str] | None) -> set[str] | None:
    if paths is None:
        return None
    return {
...
def _should_track_directory(path: str, tracked: set[str] | None) -> bool:
...
def execute_transfer(
    ...
    track_moved_directories: Collection[str] | None = None,
) -> OperationReport:
    """Execute a prepared copy/move plan without any UI interaction."""
    plan = _as_sequence(items)
    report = OperationReport()
    total = len(plan)
    tracked = _tracked_path_set(track_moved_directories)
    ...
    for index, item in enumerate(plan, start=1):
        ...
        try:
            ...
            if move:
                ...
                if item.is_directory and _should_track_directory(item.source, tracked):
                    report.record_directory_change(item.source, item.destination)
            else:
...
def _mounted_filesystems() -> tuple[str, ...]:
    ...
    seen: set[str] = set()
    try:
        handle = Path("/proc/self/mountinfo").open(
...
def _record_completed_directory(
    report: OperationReport,
    path: str,
    ...
    if was_directory and _should_track_directory(path, tracked):
        report.record_directory_change(path, destination)


def _gio_trash_batch(
    gio: str,
    ...
    """Trash one bounded path batch, falling back item-by-item on failure."""

    if not entries:
        return
    try:
        subprocess.run(
            [gio, "trash", *(path for path, _was_directory in entries)],
            check=True,
        ...
        # GIO can process part of a batch before returning a failure. Missing
        # sources are therefore counted as successfully trashed before the
        # remaining paths use the freedesktop fallback.
        for path, was_directory in entries:
            try:
                if os.path.lexists(path):
                    _manual_trash(path, trash_files, trash_info)
                report.completed += 1
                _record_completed_directory(
                    report, path, None, was_directory, tracked
                )
            except Exception as exc:
                report.add_error(f"{path}: {exc}")
    else:
        report.completed += len(entries)
        for path, was_directory in entries:
            _record_completed_directory(report, path, None, was_directory, tracked)
...
def trash_paths(
    ...
    Ordinary paths are handed to GIO in bounded batches rather than spawning a
    process for every selected item. The batch has both an item and encoded-byte
    limit, keeping command memory bounded and well below typical ARG_MAX values.
    """

    targets = _as_sequence(paths)
    total = len(targets)
    _root, trash_files, trash_info = trash_directories(trash_root)
    gio = shutil.which("gio") if trash_root is None else None
    report = OperationReport()
    tracked = _tracked_path_set(track_moved_directories)
    gio_batch: list[tuple[str, bool]] = []
    gio_batch_bytes = 0

    def flush_gio_batch() -> None:
        nonlocal gio_batch, gio_batch_bytes
        if gio is not None and gio_batch:
            _gio_trash_batch(
                gio,
                gio_batch,
                trash_files,
    ...
    for index, raw_path in enumerate(targets, start=1):
        path = os.path.abspath(os.path.expanduser(str(raw_path)))
        _emit_progress(progress_callback, index, total, os.path.basename(path))
        ...
        try:
            ...
            if gio_batch and (
                len(gio_batch) >= TRASH_BATCH_MAX_ITEMS
                or gio_batch_bytes + encoded_size > TRASH_BATCH_MAX_BYTES
            ):
                flush_gio_batch()
            gio_batch.append((path, was_directory))
            gio_batch_bytes += encoded_size
        except Exception as exc:
            report.add_error(f"{path}: {exc}")
...
def delete_paths(
    paths: Iterable[str],
    progress_callback: ProgressCallback = None,
    track_deleted_directories: Collection[str] | None = None,
) -> OperationReport:
    """Permanently delete a bounded bulk selection outside the UI thread."""

    targets = _as_sequence(paths)
    total = len(targets)
    report = OperationReport()
    tracked = _tracked_path_set(track_deleted_directories)
    ...
    for index, raw_path in enumerate(targets, start=1):
        ...
        try:
            if not os.path.lexists(path):
                raise FileNotFoundError("item no longer exists")
            trash_location = _trash_location_for_path(path)
            if trash_location is not None:
                containing_files, containing_info = trash_location
                if os.path.abspath(path) == os.path.abspath(containing_files):
                    raise ValueError("the Trash container itself cannot be deleted")
...
def _empty_desktop_trash_with_gio(gio: str) -> OperationReport:
    # A very large Trash listing is spooled to disk instead of being retained as
    # one Python string merely to count its lines.
    with tempfile.TemporaryFile(mode="w+b") as listing:
        ...
        listing.seek(0)
        entry_count = sum(1 for line in listing if line.strip())
    subprocess.run(
        [gio, "trash", "--empty"],
        check=True,
        stdout=subprocess.DEVNULL,
