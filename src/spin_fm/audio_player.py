...
class AudioPlayerWidget(QFrame):
    ...
    def _ensure_backend(self) -> bool:
        ...
        if seekable_changed is not None and hasattr(seekable_changed, "connect"):
            try:
                seekable_changed.connect(self._seekability_changed)
            except Exception:
                pass
    ...
    def _seekability_changed(self, *_args) -> None:
        self._refresh_seek_controls()
    ...
    def _release_backend(self) -> None:
        ...
        if player is not None:
            signal_slots = (
                ("positionChanged", self._position_changed),
                ("durationChanged", self._duration_changed),
                ("playbackStateChanged", self._state_changed),
                ("stateChanged", self._state_changed),
                ("seekableChanged", self._seekability_changed),
                ("errorOccurred", self._playback_error),
