...
class MainWindow(QMainWindow):
    ...
    @classmethod
    def _bound_pixmap_cache(cls) -> None:
        """Respect small platform defaults while capping oversized icon caches."""
        try:
            current_limit = int(QPixmapCache.cacheLimit())
            if current_limit > cls.MAX_PIXMAP_CACHE_KB:
                QPixmapCache.setCacheLimit(cls.MAX_PIXMAP_CACHE_KB)
        except Exception:
            pass
    ...
    def _build_status_bar(self) -> None:
        ...
        self.copy_mode_label = QLabel("", self.status_bar)
        self.copy_mode_label.setObjectName("copyModeStatusLabel")
        self.copy_mode_label.setToolTip("Hold Ctrl while dragging to copy instead of move")
        self.copy_mode_label.hide()
        self.status_bar.addPermanentWidget(self.copy_mode_label)
    ...
    def _set_copy_mode_indicator(self, enabled: bool) -> None:
        enabled = bool(enabled)
        if enabled == self._copy_mode_enabled:
            return
        self._copy_mode_enabled = enabled
        label = getattr(self, "copy_mode_label", None)
        if label is None:
            return
        if enabled:
            label.setText("Copy mode enabled")
            label.show()
        else:
            label.clear()
            label.hide()
    ...
    def _key_code(value) -> int:
        """Normalize PyQt5 integers and PyQt6 enum values to one key code."""
        try:
            return int(value)
    ...
    def _event_targets_active_file_view(self, watched=None) -> bool:
        """Recognize the current view and its viewport/focused descendants."""
        try:
            view = self.tabs.currentView()
        except Exception:
            return False
        if view is None:
            return False
        ...
        if focus is not None:
            try:
                return bool(view.isAncestorOf(focus))
            except (RuntimeError, TypeError):
                pass
        return False

    def _active_file_view_has_focus(self) -> bool:
        return self._event_targets_active_file_view()

    def eventFilter(self, watched, event):  # noqa: N802 - Qt API name
        event_type = event.type()
        if event_type in (QEvent.KeyPress, QEvent.KeyRelease):
            try:
                if event.key() == Qt.Key_Control and self.isActiveWindow():
                    if not event.isAutoRepeat():
                        self._set_copy_mode_indicator(event_type == QEvent.KeyPress)
            except Exception:
                pass

        if event_type in (QEvent.ApplicationDeactivate, QEvent.WindowDeactivate):
            self._set_copy_mode_indicator(False)

        # Return/Enter can be delivered to QListView's viewport and consumed by
        # style-dependent item-view handling before FileIconListView.keyPressEvent
        # runs. Claim ShortcutOverride and KeyPress at application level whenever
        # the active file view (or its viewport) is the event target.
        if event_type in (QEvent.ShortcutOverride, QEvent.KeyPress):
            try:
                if (
                    self._is_enter_key(event)
                    and QApplication.activeModalWidget() is None
                    and self._event_targets_active_file_view(watched)
                ):
                    event.accept()
                    if event_type == QEvent.ShortcutOverride:
                        return True
                    if not event.isAutoRepeat():
                        self.tabs.activateCurrentSelection()
                    return True
            except Exception:
                pass

        # Keep Delete reliable even when a child viewport consumes shortcut
        # activation. Only handle it for the active file view and never while a
        ...
        if event_type == QEvent.KeyPress:
            try:
                if (
                    ...
                    and QApplication.activeModalWidget() is None
                    and self._active_file_view_has_focus()
                ):
                    event.accept()
                    permanent = bool(event.modifiers() & Qt.ShiftModifier)
                    self.tabs.deleteSelection(permanent=permanent)
                    return True
            except Exception:
                pass
    ...
    def _sidebar_splitter_moved(self, _position: int, _index: int) -> None:
        if self._restoring_window_state or self._closing:
            return
    ...
    def _restore_window_state(self) -> None:
        ...
        splitter_was_blocked = self.splitter.blockSignals(True)
        try:
            ...
            self.splitter.blockSignals(splitter_was_blocked)
            self._restoring_window_state = False
        ...
        QTimer.singleShot(0, self._remember_sidebar_width)
