...
* a single click leaves the location bar on the current directory;
...
MAX_SCANNED_DIRECTORIES = 50_000
                ...
                for entry in entries:
                    ...
                    if stat.S_ISDIR(entry_info.st_mode):
                        ...
                        if identity in seen_directories:
                            skipped_items += 1
                            continue
                        if len(seen_directories) >= MAX_SCANNED_DIRECTORIES:
                            # Keep both the loop-protection set and pending stack
                            # bounded on exceptionally broad directory trees.
                            skipped_items += 1
                            continue
...
class FileInfoExtension:
    ...
    def __init__(self, main_window: Any) -> None:
        ...
        # Owned single-shot timers coalesce repeated zero-delay callbacks. This
        # avoids accumulating bound-method events during rapid tab changes or
        # repeated inspection completions while keeping all work on the UI thread.
        self._view_bind_timer = QTimer(main_window)
        self._view_bind_timer.setSingleShot(True)
        self._view_bind_timer.timeout.connect(self._bind_all_views)
        self._pending_start_timer = QTimer(main_window)
        self._pending_start_timer.setSingleShot(True)
        self._pending_start_timer.timeout.connect(self._start_pending_request)
    ...
    def _view_by_id(self, view_id: int) -> Any | None:
        """Find a live tab view without retaining a non-weakrefable Qt wrapper."""
        try:
    ...
    def _schedule_view_bind(self) -> None:
        """Queue at most one deferred scan for newly created tab views."""
        if self._shutting_down:
            return
        try:
            if not self._view_bind_timer.isActive():
                self._view_bind_timer.start(0)
        except (AttributeError, RuntimeError):
            pass

    def _schedule_pending_start(self) -> None:
        """Queue at most one deferred attempt to start pending inspection."""
        if self._shutting_down:
            return
        try:
            if not self._pending_start_timer.isActive():
                self._pending_start_timer.start(0)
        except (AttributeError, RuntimeError):
            pass
    ...
    def _bind_view(self, view: Any) -> bool:
        ...
        try:
            ...
            # Some older PyQt/SIP wrappers cannot be weak-referenced. Resolve the
            # wrapper through the tab widget instead of keeping it alive forever.
            def tab_lookup(bound_id: int = view_id) -> Any | None:
                return self._view_by_id(bound_id)

            view_reference = tab_lookup
    ...
    def _disconnect_all_views(self) -> None:
        """Disconnect tab callbacks without duplicating the complete registry."""
        while self._view_slots:
            _view_id, record = self._view_slots.popitem()
            view_reference, clicked, destroyed = record
            view = view_reference()
            if view is None:
                continue
            try:
    ...
    def _on_current_tab_changed(self, _index: int) -> None:
        ...
        self._clear_selected_item_state()
    ...
    def _clear_selected_item_state(self) -> None:
        """Release metadata retained for a selection that is no longer current."""
        self.filepath = ""
        self.info = None
        self.basic = ""

    def on_treeview2_clicked(self, index: Any, view: Any | None = None) -> None:
        """Inspect a selected item without changing the active directory."""
        if self._shutting_down:
            return
        ...
        # Do not retain the previous item's stat payload while a replacement is
        # being inspected. The visible status remains until the new result arrives.
        self.info = None
    ...
    def shutdown(self) -> None:
        ...
        for timer in (self._view_bind_timer, self._pending_start_timer):
            try:
                timer.stop()
        ...
        tasks = self._tasks
        try:
            tasks.shutdown(wait_msec=2_000)
        except Exception:
        ...
        # Break the extension/window ownership cycle immediately rather than
        # relying on a later Python collection pass after the Qt window closes.
        window = self.window
        try:
            if getattr(window, self.ATTRIBUTE_NAME, None) is self:
                delattr(window, self.ATTRIBUTE_NAME)
        except Exception:
            pass
