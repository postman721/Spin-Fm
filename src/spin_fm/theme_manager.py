...
class ThemeManager:
    ...
    def _read_stylesheet(self, path: Path) -> str:
        ...
        if cached is not None and cached[0] == stat.st_mtime_ns:
            # Dicts preserve insertion order; move a hit to the end for a tiny LRU.
            self._stylesheet_cache.pop(path, None)
            self._stylesheet_cache[path] = cached
            return cached[1]
        stylesheet = path.read_text(encoding="utf-8")
        self._stylesheet_cache[path] = (stat.st_mtime_ns, stylesheet)
        while len(self._stylesheet_cache) > self.MAX_STYLESHEET_CACHE_ITEMS:
            oldest = next(iter(self._stylesheet_cache))
            self._stylesheet_cache.pop(oldest, None)
        return stylesheet
