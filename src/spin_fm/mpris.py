...
class MPRISService(QObject):
    ...
    @staticmethod
    def _delete_adaptors(*adaptors: Any) -> None:
        """Break adaptor back-references and release their QObject wrappers."""
        for adaptor in adaptors:
            if adaptor is None:
                continue
            try:
                adaptor._service = None
            except Exception:
                pass
            try:
                adaptor.setParent(None)
            except Exception:
                pass
            try:
                adaptor.deleteLater()
    ...
    def _register(self) -> bool:
        ...
        connection: Any = None
        root_adaptor: Any = None
        player_adaptor: Any = None
        service_name = ""
        ...
        try:
            ...
            if not object_registered:
                self._delete_adaptors(root_adaptor, player_adaptor)
                return False
            ...
            if connection is not None and object_registered:
                try:
                    ...
                    pass
            if connection is not None and service_name:
                try:
    ...
    def _unregister(self) -> None:
        ...
        root_adaptor = self._root_adaptor
        player_adaptor = self._player_adaptor
